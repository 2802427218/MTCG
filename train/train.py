...
def parse_arguments():
    ...
    parser.add_argument("--attribute_classify_loss", type=float, default=0.2)
...
def wandb_logging(args):
    if args.wandb:
        wandb.login(key="1b669e422295adc8e02a3583c599daab3882b553")
        wandb.init(project="MCTG_trainer")


def main():

    global training_args, train_dataset
    args = parse_arguments()
    print(args)
    encoder, decoder, encoder_tokenizer, decoder_tokenizer = load_models_and_tokenizers(args=args)
    model = MTCGModel(encoder=encoder, decoder=decoder, args=args)
    wandb_logging(args)

    loss_list = {}
    attribute_classify_args = None
    ...
    if args.attribute_classify_loss is not None:
        loss_list['attribute_classify_loss'] = args.attribute_classify_loss
        attribute_classify_args = {
    ...
    model.set_losslist(loss_list, attribute_classify_args, aspect_gap_args)
    ...
    if args.classifier_head_type == 'multiple':
        ...
        columns = ['encoder_input_ids', 'encoder_attention_mask', 'encoder_token_type_ids', 'type', 'decoder_input_ids',
                   'decoder_attention_mask']
        if 'contrasitive_loss' in loss_list:
            columns.extend(['adv_input_ids', 'adv_attention_mask', 'adv_token_type_ids'])
        if 'attribute_classify_loss' in loss_list:
        ...
        for i in range(3):
            ...
            tmp_dataset = tmp_dataset.map(
                lambda e: encoder_tokenizer(e['sent'], max_length=args.max_length, padding='max_length',
            ...
            tmp_dataset = tmp_dataset.map(
                lambda e: decoder_tokenizer(e['sent'], max_length=args.max_length, padding='max_length',
                                            truncation=True), batched=True)
            tmp_dataset = tmp_dataset.rename_columns(
                {'input_ids': 'decoder_input_ids', 'attention_mask': 'decoder_attention_mask'})
            if 'contrasitive_loss' in loss_list:
                tmp_dataset = tmp_dataset.map(
                    lambda e: encoder_tokenizer(e['adv_sent'], max_length=args.max_length, padding='max_length',
                                                truncation=True), batched=True)
                tmp_dataset = tmp_dataset.rename_columns(
                    {'input_ids': 'adv_input_ids', 'attention_mask': 'adv_attention_mask',
                     'token_type_ids': 'adv_token_type_ids'})
            if 'attribute_classify_loss' in loss_list:
        ...
        train_dataset = train_dataset.map(
            lambda e: encoder_tokenizer(e['sent'], max_length=args.max_length, padding='max_length', truncation=True),
            batched=True)
        train_dataset = train_dataset.rename_columns(
            {'input_ids': 'encoder_input_ids', 'attention_mask': 'encoder_attention_mask',
             'token_type_ids': 'encoder_token_type_ids'})
        train_dataset = train_dataset.map(
            lambda e: decoder_tokenizer(e['sent'], max_length=args.max_length, padding='max_length', truncation=True),
            batched=True)
        train_dataset = train_dataset.rename_columns(
            {'input_ids': 'decoder_input_ids', 'attention_mask': 'decoder_attention_mask'})
        columns = ['encoder_input_ids', 'encoder_attention_mask', 'encoder_token_type_ids', 'type', 'decoder_input_ids',
