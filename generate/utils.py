...
class KCenters:
    ...
    def optim(self, centers, matrix, topk, strategy, batch=100, temperature=50):
        ...
        for i in range(epoch):
            ...
            if strategy == 'none':
                tmp_centers = torch.mean(torch.gather(matrix.unsqueeze(0).expand(end - start, -1, -1), 1,
                                                      tmp_indices[start:end].unsqueeze(-1).expand(-1, -1,
                                                                                                  self.latent_size)),
                                         dim=1).squeeze()
                ...
                tmp_c = torch.gather(matrix.unsqueeze(0).expand(end - start, -1, -1), 1,
                                     tmp_indices[start:end].unsqueeze(-1).expand(-1, -1, self.latent_size))
